...
- [ ] **P1.1** — Implement `polylabs_app_graph.fl` with 12 module definitions and `make_polylabs_module` helper
- [ ] **P1.2** — Wire 18 `EDGE_REQUIRES` dependency edges in `polylabs_app_graph_register`
- [ ] **P1.3** — Implement `polylabs_register_bridge_edges` (4 bridges: PolyKit metering, PolyKit RBAC, SPARK, scatter-cas)
- [ ] **P1.4** — Implement `polylabs_register_governance_edges` (12 `EDGE_GOVERNANCE_OBSERVE`)
- [ ] **P1.5** — Add `find_polylabs_module_id` helper and `polylabs_app_graph_metrics` store
...
- [ ] **P4.1** — Verify all 12 modules resolve via `find_polylabs_module_id` after registration
- [ ] **P4.2** — Verify bridge edges connect to live PolyKit and eStream module graphs
...
1. `polylabs_app_graph_register` produces a `CsrStorage` with exactly 12 nodes and 18 edges
2. `polylabs_register_bridge_edges` adds 4 bridge nodes with `EDGE_BRIDGE_TO` edges
3. `polylabs_register_governance_edges` adds 12 `EDGE_GOVERNANCE_OBSERVE` edges
...
| File | Description |
|------|-------------|
| `circuits/fl/polylabs_app_graph.fl` | 12-module app graph, edges, bridges, governance, tests |
| `circuits/fl/polylabs_meaning.fl` | 3 CE domains, noise filter, 2 SME panels, tests |
